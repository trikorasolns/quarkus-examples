...
//            Panache.withTransaction(() -> Multi.createFrom().items(new Fruit("pear", "Pear").persist(), new Fruit("apple", "Apple").persist()).collect().asList().map(unis -> {List::add}).toUni().await().atMost(Duration.ofSeconds(30)));
    }
  }
