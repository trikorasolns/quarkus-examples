...
import com.trikorasolutions.example.resource.UserResource;
...
//    }
    ...
    List<String> responseList  =
      RestAssured.given().auth().oauth2(getAccessToken("alice"))
        .when().contentType(MediaType.APPLICATION_JSON).get("/api/users/roles")
        .then().statusCode(200).contentType(MediaType.APPLICATION_JSON)
...
//      .statusCode(200);
  ...
  public void testInfo() {
    ...
    RestAssured.given().auth().oauth2(getAccessToken("jdoe"))
      .when().contentType(MediaType.APPLICATION_JSON).get("/api/users/userinfo")
      .then().statusCode(200).contentType(MediaType.APPLICATION_JSON)
    ...
    RestAssured.given().auth().oauth2(getAccessToken("jdoe"))
      .when().contentType(MediaType.APPLICATION_JSON).get("/api/users/userinfo")
      .then().statusCode(200).contentType(MediaType.APPLICATION_JSON)
      .extract().response().jsonPath().getList("attributes"),
